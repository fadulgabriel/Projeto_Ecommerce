...
format:
  html:
    ...
    smooth-scroll: true
    code-fold: true
    code-summary: "Ver Código Fonte (R)"
    self-contained: true
execute:
  warning: false
  message: false
...
Este projeto analisa a base pública de dados da **Olist**, o maior ecossistema de e-commerce do Brasil, com o objetivo de mapear como a eficiência logística impacta diretamente a retenção e a satisfação do cliente.

**Principais Descobertas:** 1. Atrasos na entrega possuem uma **correlação negativa estatisticamente comprovada** com a satisfação do consumidor. 2. O engajamento via comentários segue uma curva em "U", onde o **ódio engaja mais que o amor**: clientes insatisfeitos fazem questão de justificar sua dor em texto muito mais do que clientes encantados.

------------------------------------------------------------------------

# Preparação dos Dados

Para garantir o rigor analítico, a base de dados passou por um rigoroso processo de limpeza. Tratamos o efeito de *Cart Splitting* (pedidos com múltiplos itens gerando duplicação de frete) agrupando os dados por `order_id`.
...
# A Dor do Cliente: Análise de Atrasos e Avaliações
